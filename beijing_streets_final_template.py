...
使用说明：
1. 准备数据：
   - 使用 beijing_streets_data_template.csv 作为模板
   - 填写你自己的街道故事数据
   - 将CSV文件另存为Excel格式（.xlsx）

2. 获取GeoJSON数据：
   - 本项目使用的GeoJSON数据来自areacity（有版权限制）
   - 你需要自行购买或获取合法的GeoJSON数据
   - 将数据放在项目文件夹的相应位置

3. 运行程序：
   - 双击"更新地图.command"（Mac）或"更新地图.bat"（Windows）
   - 或者直接运行此Python脚本
"""
...
class BeijingStreetsFinalMap:
    def __init__(self, excel_file='beijing_streets_data.xlsx', 
        ...
        Args:
            ...
            geojson_folder: GeoJSON数据文件夹路径，如果为None则使用相对路径
        """
        self.excel_file = excel_file
        
        # 如果未提供geojson_folder，使用通用的相对路径
        if geojson_folder is None:
            import os
            script_dir = os.path.dirname(os.path.abspath(__file__))
            self.geojson_folder = os.path.join(script_dir, 'geojson_data')
        else:
    ...
    def show_data_summary(self):
        ...
        if self.stories_data is not None:
            ...
            for fate in sorted(fate_distribution.keys()):
                ...
                print(f"  缘分{fate}分: {count} 个街道 ({percentage:.1f}%)")
        else:
            print("数据尚未加载")
    ...
    def run(self):
        ...
        # 7. 询问是否在浏览器中打开
        print("\n" + "=" * 70)
        choice = input("是否在浏览器中打开地图？(y/n): ").strip().lower()
        if choice == 'y':
            self.open_in_browser(output_file)
        
        print("\n🎉 地图生成完成！")
        print(f"📁 文件位置: {os.path.abspath(output_file)}")
        print("💡 提示：你可以随时修改Excel文件，然后重新运行此程序更新地图")
